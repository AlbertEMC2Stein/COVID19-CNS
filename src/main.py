...
class Simulation:
    ...
    def start_iteration(self):
        ...
        vacs = self.settings["vaccinations"]
        ...
        for ini_rec in ini_recs:
            _ = ini_rec.make_immune(np.random.poisson(c_immunity))
            self.groups["Recovered"].add_member(ini_rec)
            ini_rec.recovered = True

        for ini_vac in ini_vacs:
            _ = ini_vac.make_immune(np.random.poisson(c_vac_immunity))
        ...
        while True:
            ...
            print("Day: %d, #Infected: %d, #newInf: %d, #newVac: %d, 7di: %d" % (tick, self.groups["Infected"].size,
                                                                   self.stats["#new_infected"][-1],
                                                                   self.stats["#new_vaccinated"][-1],
                                                                   self.stats["seven_day_incidence"][-1]))
...
if __name__ == "__main__":
    simulation_settings = {
        ...
        "immunity_time": 180,
        "number_of_initially_infected": 1000,
        "number_of_initially_recovered": 2000,
        ...
        "maximal_simulation_time_interval": 1826 # 3652
    }
    ...
    plt.plot(106463 - data[:, 0] - data[:, 1] - data[:, 2], color='green')
    plt.plot(data[:, 0], color='red')
    plt.plot(data[:, 1], color='blue')
    plt.plot(data[:, 2], color = "cyan")
    plt.show()

    plt.plot(sim.stats["#new_infected"], color = "red")
    plt.title("New Infections")
    plt.show()

    plt.plot(data[:, 0], color='red')
    plt.plot(data[:, 1], color='blue')
    plt.title("Infected & Recovered")
    plt.show()
    ...
    plt.plot(sim.stats["seven_day_incidence"], color = "red")
    plt.title("Seven Day Incidence")
    plt.show()
